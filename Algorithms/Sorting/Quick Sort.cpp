/**
 * Quick Sort O(n log n) - Sắp xếp nhanh
 * Xem thông tin về các thuật toán sắp xếp: https://visualgo.net/en/sorting
 * https://youtu.be/PgBzjlCcFvc
 * Giải thuật sắp xếp nhanh chia mảng thành hai phần bằng cách so sánh từng
 * phần tử của mảng với một phần tử được chọn gọi là phần tử chốt (Pivot): một
 * mảng bao gồm các phần tử nhỏ hơn hoặc bằng pivot và mảng còn lại bao gồm các
 * phần tử lớn hơn hoặc bằng pivot. Tiến trình chia này diễn ra tiếp tục cho tới
 * khi độ dài của các mảng con đều bằng 1. Giải thuật sắp xếp nhanh tỏ ra khá
 * hiệu quả với các tập dữ liệu lớn khi mà độ phức tạp trường hợp trung bình
 * và trường hợp xấu nhất là O(n log n)
*/
#include<iostream>
using namespace std;
...
int partition(int arr[], int low, int high)
{
    ...
    while(true)
    {
        ...
        // Tăng left và giảm right 1 do chúng vừa được hoán đổi ở bước trên
        left++, right--;
    }

    // Đổi chỗ phần tử ở vị trí left cho phần tử ở vị trí hight
